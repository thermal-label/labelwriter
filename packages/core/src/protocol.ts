...
/**
 * Fit the authored bitmap to the engine's head width and emit it
 * verbatim in feed direction. Pad right with white when narrower than
 * `headDots`, crop right when wider. **No dead-zone handling** —
 * leading/trailing/left/right offsets are a property of the *authoring
 * canvas*, not the wire format. Callers (the harness, drivers) size
 * their bitmaps via `getPrintableCanvasDots(engine, media)` so the
 * authored region already matches the head's reachable area; the
 * encoder then trusts every row.
 *
 * Historical context: an earlier iteration (plan 08 §6) cropped the
 * top `leading` mm of rows here, on the assumption that the head sits
 * past the leading edge after form-feed. That silently ate authored
 * content. Bench testing on debug/print-flow (2026-05-23) confirmed
 * the right fix is to expose the dead zones to the authoring layer,
 * not to mutate the bitmap at encode time.
 */
function composeWireBitmap(bitmap: LabelBitmap, engine: PrintEngine): LabelBitmap {
...
export function encodeLabel(
  ...
  // Width-only fit: pad/crop to `headDots`, emit every row verbatim.
  // Dead-zone offsets are an authoring-canvas property exposed via
  // `getPrintableCanvasDots(engine, media)`; the encoder trusts whatever
  // bitmap height the caller supplied.
  const fitted = composeWireBitmap(bitmap, engine);
